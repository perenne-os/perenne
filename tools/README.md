...
| `check-references.ps1` | Validates doc cross-references: every `KB-####` id, every root-relative `docs/`/`knowledge-base/` path, and every Markdown link target (`.md`) mentioned must actually resolve. Skips historical snapshots under `docs/design/`. |
