...
- **SSH signature *verification*:** with signing working (above), `git log --show-signature` still printed `error: gpg.ssh.allowedSignersFile needs to be configured` and reported `No signature` — even though the commit *was* signed. Verifying SSH signatures needs an allowed-signers file mapping the committer email to the trusted public key. Fix: create `C:/Users/<you>/.ssh/allowed_signers` (one line: `<email> <contents of your .pub key>`) and run `git config --global gpg.ssh.allowedSignersFile "C:/Users/<you>/.ssh/allowed_signers"`. (Knowledge base: `knowledge-base/entries/KB-0004.md`.) **Signing, transport, and verification are three separate concerns — a commit can be signed yet unverifiable until the verifier has a trust anchor.**
